...
cryptogen extend --input organizations --config ./addOrderer/crypto-config.yaml

peer channel fetch config ./addOrderer/config_block.pb -o orderer.example.com:7050 -c system-channel --tls --cafile ${ORDERER_CA}

#pwd addOrderer
configtxlator proto_decode --input config_block.pb --type common.Block > config.json
cat config.json | jq .data.data[0].payload.data.config > test.json
mv test.json config.json
export TLS_FILE=${PWD}/../organizations/ordererOrganizations/example.com/orderers/orderer1.example.com/tls/server.crt
echo "{\"client_tls_cert\":\"$(cat $TLS_FILE | base64)\",\"host\":\"orderer1.example.com\",\"port\":7060,\"server_tls_cert\":\"$(cat $TLS_FILE | base64)\"}" > orderer1consenter.json
...
peer channel update -f ./addOrderer/config_update_in_envelope.pb -c system-channel -o orderer.example.com:7050 --tls true --cafile ${ORDERER_CA}

peer channel fetch config ./addOrderer/latest_config.block -o orderer.example.com:7050 -c system-channel --tls --cafile ${ORDERER_CA}
```
